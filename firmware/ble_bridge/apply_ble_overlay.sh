...
perl -0pi -e 's@return audioOpen \? MODE_RX : MODE_STOPPED;@return (audioOpen || (desiredState.flags & HOST_STATE_APRS_WEAK_RX)) ? MODE_RX : MODE_STOPPED;@' "${SKETCH}"
perl -0pi -e 's@uint16_t desiredFilterFlags\(\) \{\n  return desiredState.flags & \(HOST_STATE_FILTER_PRE \| HOST_STATE_FILTER_HIGH \| HOST_STATE_FILTER_LOW\);\n\}@uint16_t desiredFilterFlags() {\n  return desiredState.flags \& (HOST_STATE_FILTER_PRE | HOST_STATE_FILTER_HIGH | HOST_STATE_FILTER_LOW);\n}\n\nbool kv4pAprsWeakRxRequested() {\n  return (desiredState.flags \& HOST_STATE_APRS_WEAK_RX) \&\& ((desiredState.flags \& HOST_STATE_PTT_REQUESTED) == 0);\n}\n\nuint16_t kv4pEffectiveFilterFlags() {\n  return kv4pAprsWeakRxRequested() ? 0 : desiredFilterFlags();\n}\n\nuint8_t kv4pEffectiveSquelch() {\n  return kv4pAprsWeakRxRequested() ? 0 : desiredState.squelch;\n}@' "${SKETCH}"
perl -0pi -e 's@appliedState.squelch != desiredState.squelch@appliedState.squelch != kv4pEffectiveSquelch()@' "${SKETCH}"
perl -0pi -e 's@uint16_t filterFlags = desiredFilterFlags\(\);@uint16_t filterFlags = kv4pEffectiveFilterFlags();@' "${SKETCH}"
perl -0pi -e 's@sa818.group\(desiredState.bw, desiredState.freq_tx, desiredState.freq_rx, desiredState.ctcss_tx, desiredState.squelch, desiredState.ctcss_rx\)@sa818.group(desiredState.bw, desiredState.freq_tx, desiredState.freq_rx, desiredState.ctcss_tx, kv4pEffectiveSquelch(), desiredState.ctcss_rx)@' "${SKETCH}"
perl -0pi -e 's@appliedState.squelch = desiredState.squelch;@appliedState.squelch = kv4pEffectiveSquelch();@' "${SKETCH}"

perl -0pi -e 's|void loop\(\) \{|void kv4pBleLoop() {\n  kv4pBleParser.loop();\n  kv4pBleTncParser.loop();\n  const bool ready = kv4pBleBridge.isReady();\n  kv4pBleRxAudioOnly = ready;\n  if (!ready && kv4pBleWasConnected) {\n    // BLE sessions own only transient live-audio/PTT flags. Clear them on\n    // disconnect so a stale iPhone session cannot keep ADC, ADPCM encoding,\n    // USB KISS audio output, power-save RX, or TX state running after the\n    // central is gone.\n    desiredState.flags &= ~(HOST_STATE_PTT_REQUESTED \| HOST_STATE_RX_AUDIO_OPEN \| HOST_STATE_RX_POWER_SAVE \| HOST_STATE_RX_POWER_SAVE_MAX \| HOST_STATE_APRS_WEAK_RX);\n    kv4pBleUrgentPttOffPending = true;\n    reconcileDesiredState(false);\n    markDeviceStateDirty();\n  }\n  if (ready && !kv4pBleWasConnected) {\n    sendHello(FIRMWARE_VER, radioModuleStatus, USB_BUFFER_SIZE, hw.rfModuleType, moduleMinRadioFreq(), moduleMaxRadioFreq(), getFirmwareFeatures(), currentDeviceState());\n  }\n  kv4pBleWasConnected = ready;\n}\n\nvoid loop() {|' "${SKETCH}"
